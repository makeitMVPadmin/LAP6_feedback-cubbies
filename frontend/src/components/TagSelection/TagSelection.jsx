...
const TagSelection = ({ selectedTags, onChange, showError }) => {
  ...
  const handleTagClick = (tag, category, e) => {
    ...
    const updatedSelectedTags = selectedTags.some(
      (t) => t.id === tag.id && t.category === category
    )
      ? selectedTags.filter(
          (t) => !(t.id === tag.id && t.category === category)
        )
      : [...selectedTags, { ...tag, category }];

    // Notify the parent component to update the selected tags
        ...
        return (
          <div key={category} className="relative w-[220px] mb-[215px]">
            ...
            <button
              ...
              className={`flex justify-between items-center bg-[#fffefe] text-black px-4 py-2 w-[200px] h-[40px] flex-shrink-0 
                border-black border-t-[1px] border-r-[2px] border-b-[2px] border-l-[1px] 
                border border-gray-800 text-base font-bold
                ${isOpen ? "rounded-t-lg border-b-[0.5px] border-b-[#d9d9d9]" : "rounded-lg"} 
            ...
            {/* dropdown Menu */}
            {isOpen && (
              <div className="absolute left-0 w-[200px] bg-white border-black border-t-0 border-r-[2px] border-b-[2px] border-l-[1px] rounded-b-lg z-10">
